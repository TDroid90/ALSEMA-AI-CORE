...
async def seed_rbac(session: AsyncSession) -> None:
    ...
    for slug, name in (
        ...
        if slug not in roles:
            session.add(Role(slug=slug, name=name, is_system_role=True))
        ...
        existing_codes = set(
            (
                await session.execute(
                    ...
                    .where(role_permissions.c.role_id == roles_by_slug[slug].id)
                )
            ).scalars()
        )
        for code in codes - existing_codes:
            await session.execute(
                role_permissions.insert().values(
                    role_id=roles_by_slug[slug].id, permission_id=permissions[code].id
                )
            )
    await session.commit()
